...
public class SeleniumScenarioTest {
    ...
    public void test_edit_10pt() {
        ...
        AddOrEditReservation editPage = AddOrEditReservation.getEditPage(this.driver, listPage.getEditButtons().get(itemNum - 1));
        Reservation reservation = reservations.get(itemNum - 1);
        editPage.assertEditFormIsPrefilled(reservation.getGuestName(), reservation.getDateCreated().toString(), reservation.getDaysOfStay().toString(), reservation.getRoomType().name(), reservation.getHotel().getId().toString());

        listPage = AddOrEditReservation.update(this.driver, editPage, "testName", LocalDate.now().minusYears(30).toString(), "0", RoomType.SINGLE.name(), hotels.get(0).getId().toString());
        listPage.assertNoError();
